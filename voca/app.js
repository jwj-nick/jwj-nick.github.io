...
(function () {
  ...
  // boost = 보강 점검에서 약하게 나온 아래 구간(신규의 일부를 여기서 뽑는다). skipped = 승급으로 건너뛴 미학습 항목.
  ...
  function freshState() {
    return {
      v: 2, track: CFG.track, profile: null, placed: false,
      level: { focus: CFG.defaultBand || 1, passed: [], boost: null, lastCheck: null, history: [] },
      courses: defaultCourses(), skipped: {}, stats: { bands: {} },
      items: {}, known: {}, seen: {}, saved: [], days: {}, lastDone: null, streakDays: 0,
      settings: { accent: CFG.defaultAccent || "us", dailyNew: A.dailyNew, auto: true }
    };
  }
  function focusBand() { return (S.level && S.level.focus) || CFG.defaultBand || 1; }
  // v1 트랙 id → v2 통합 id. 대응표에 없는 id는 그대로 유효하다.
  function remapId(id) { return LEGACY[id] || id; }
  function remapKeys(obj) {
    if (!obj) return obj;
    var out = {};
    Object.keys(obj).forEach(function (k) { var nk = remapId(k); if (out[nk] === undefined) out[nk] = obj[k]; });
    return out;
  }
  function migrateV1(s) {
    if (!s || s.v !== 1) return s;
    var f = freshState();
    var start = Number(s.startBand) || f.level.focus;
    s.v = 2;
    s.profile = s.profile || (s.track === "mid" || s.track === "high" || s.track === "adult" ? s.track : null);
    s.level = { focus: start, passed: [], boost: null, lastCheck: null,
                history: [{ date: todayStr(), band: start, kind: "init", ok: null, n: null, note: "v1 시작 구간" }] };
    for (var b = minBand(); b < start; b++) s.level.passed.push(b);
    s.courses = f.courses; s.skipped = {}; s.stats = { bands: {} };
    delete s.startBand;
    return s;
  }
  function migrateIds(s) {
    if (!Object.keys(LEGACY).length || !s) return 0;
    var n = 0;
    ["items", "known", "seen", "skipped"].forEach(function (k) {
      if (!s[k]) return;
      var before = Object.keys(s[k]).filter(function (id) { return LEGACY[id]; }).length;
      if (before) { s[k] = remapKeys(s[k]); n += before; }
    });
    if (s.saved) s.saved = s.saved.map(remapId).filter(function (id, i, a) { return a.indexOf(id) === i; });
    Object.keys(s.days || {}).forEach(function (d) {
      var set = s.days[d] && s.days[d].set; if (!set) return;
      ["R", "S", "G", "N", "K"].forEach(function (k) { if (set[k]) set[k] = set[k].map(remapId); });
    });
    return n;
  }
  function normalize(s) {
    ...
    s.stats = s.stats || f.stats; s.stats.bands = s.stats.bands || {};
    s.track = CFG.track;
    migrateGrad(s);
    migrateIds(s);
    return s;
  ...
  function readState(key) {
    try { var s = JSON.parse(localStorage.getItem(key)); return (s && (s.v === 1 || s.v === 2) && s.items) ? s : null; } catch (e) { return null; }
  }
  function load() {
    var s = readState(KEY), from = null;
    // 통합 배포에서 처음 열면 옛 배포(mid1·high1)의 진도를 같은 origin에서 가져온다 (config.legacyNs)
    if (!s && CFG.legacyNs && CFG.legacyNs.length) {
      for (var i = 0; i < CFG.legacyNs.length && !s; i++) { s = readState(CFG.legacyNs[i] + ".v1"); if (s) from = CFG.legacyNs[i]; }
    }
    if (!s) return freshState();
    s = normalize(migrateV1(s));
    if (from) s.migratedFrom = from;
    return s;
  }
  // 백업 파일·저장 상태에 어느 앱 버전이 썼는지 남긴다 — 나중에 상태 스키마를 옮길 때 근거가 된다.
  function save() { try { S.appVersion = CFG.appVersion || S.appVersion; localStorage.setItem(KEY, JSON.stringify(S)); } catch (e) {} }
  ...
  function stateLabel(id, t) {
    ...
    if (S.skipped && S.skipped[id]) return { cls: "", text: "건너뜀" };
    return { cls: "", text: "" };
  }
  ...
  // 현재 구간과 바로 위 구간을 한 묶음으로 섞어 내보낸다 (결정 #23의 일반화 — v2는 레벨 상대).
  // 그렇지 않으면 한 구간 400개를 다 끝낸 뒤에야 다음 구간이 나와서 두 달 넘게 한 구간만 돈다.
  ...
  function pickNew(n) {
    ...
    // 예전 저장 상태의 시작 구간이 낮아도 마찬가지. 검색으로는 여전히 담을 수 있다.
    ...
    // 그 구간이 바닥나면 자연히 풀린다.
    var boost = S.level && S.level.boost;
    if (boost && boost < floor) {
      var bq = unseenIn("word", boost);
  ...
  function buildToday(t) {
    ...
    var G = gradIds().filter(function (id) { var g = S.items[id]; return g.due && g.due <= t; })
      ...
      .slice(0, A.gMax || 0);
    // 건너뛴 단어 표본(K) — 승급으로 건너뛴 미학습 항목을 하루 kMax개만 확인한다. 틀리면 보관함으로.
    // "안다"고 치고 넘어간 것이 사라지지 않게 하는 장치(설계 §4-2).
    var K = Object.keys(S.skipped || {}).filter(function (id) { return byId[id]; })
      .sort(function (a, b) { return S.skipped[a] < S.skipped[b] ? -1 : S.skipped[a] > S.skipped[b] ? 1 : (a < b ? -1 : 1); })
      .slice(0, A.kMax === undefined ? 1 : A.kMax);
    var N = pickNew(newCount(t));
    var over = R.length + Sp.length + G.length + K.length + N.length - A.totalMax;
    if (over > 0) N = N.slice(0, Math.max(0, N.length - over));
    d.set = { R: R, S: Sp, G: G, K: K, N: N };
    d.done = false;
    save();
    return d.set;
  }
  ...
  function pickType(it, r, purpose) {
    ...
    // 졸업이 걸린 문항(s=1 → 맞히면 졸업)·졸업 후 장기 점검·레벨 점검·건너뛴 단어 표본은 산출형으로 낸다.
    // 뜻 고르기만으로 졸업하면 "보면 아는" 단계에서 학습이 끝난다. 레벨도 인식만으로는 올리지 않는다(결정 #33).
    if ((r && r.s === 1) || purpose === "recall" || purpose === "level" || purpose === "skip") {
  ...
  function startDaily() {
    ...
    sess.pendingIds = set.R.filter(function (id) { return isVault(id) && S.items[id].due <= t; })
      ...
      .concat(skipDue(set).map(function (id) { return { id: id, purpose: "skip" }; }));
  ...
  function judgeCard(known) {
    ...
    d.newStats.shown++;
    var bs = bandStat(byId[id].band); bs.shown++;
    if (known) { S.known[id] = addDays(t, A.knownDays); S.seen[id] = t; sfxCorrect(0); }
    else { toVault(id, t, 1); d.newStats.unknown++; bs.unknown++; tone(330, 0.1, "sine", 0.06); }
  ...
  function renderQuestion() {
    ...
    var label = sess.mode === "place" ? "실력 체크" : sess.mode === "level" ? "레벨 점검 · " + bandLabel(sess.level.band)
      : sess.mode === "vault" ? "보관함 재시험" : "오늘의 퀴즈";
    $("quiz-label").textContent = label + " " + (sess.idx + 1) + "/" + total + (q.purpose === "check" ? " · 오늘 배운 단어" : q.purpose === "spot" ? " · 점검"
       : q.purpose === "recall" ? " · 졸업 단어 복습" : q.purpose === "skip" ? " · 건너뛴 단어 점검" : "");
    $("q-type").textContent = TYPE_LABEL[q.type] || "";
    // 게이트(smoke)가 받아쓰기의 정답을 계산할 수 있도록 문항의 항목 id를 남긴다 — 화면에는 보이지 않는다.
    $("view-quiz").setAttribute("data-qid", q.id);
  ...
  function next() {
    ...
    // 콤보 자동 진행(650ms 타이머)과 사용자의 "다음" 탭이 겹치면 next()가 두 번 불린다.
    // 마지막 문항에서는 finishSession()이 두 번 돌아 레벨 점검 판정이 이중으로 기록됐다 — 즉시 버튼을 감춰 막는다.
    $("btn-next").classList.add("hidden");
    sess.idx++;
  ...
  function grade(q, correct) {
    ...
    if (q.purpose === "level") { // 레벨 점검 — 항목 상태는 건드리지 않는다. 판정은 finishLevelCheck에서 한 번에.
      sess.level.n++; if (correct) sess.level.ok++;
      return correct ? "정답!" : "괜찮아요";
    }
    if (q.purpose === "skip") { // 건너뛴 단어 표본 — 맞히면 본 것으로, 틀리면 보관함으로. 어느 쪽이든 표본 풀에서 빠진다.
      delete S.skipped[id];
      if (correct) { S.seen[id] = t; save(); return "건너뛴 단어 확인 ✓"; }
  ...
  // ---------- 레벨 (v2 — 20_design/v2-unified-ladder.md §4) ----------
  // 레벨은 설정값이 아니라 측정값이다. 신호(구간별 안다/모른다·첫 확인 정답)는 점검을 "제안"만 하고,
  // 레벨은 점검을 통과해야 움직인다(결정 #33). 첫 실력 체크는 프리셋 구간에서 시작해 위아래로 더듬는 적응형.
  function bandLabel(b) { return (CFG.bands && CFG.bands[String(b)]) || ("band " + b); }
  function bandStat(b) {
    var bs = S.stats.bands; return bs[b] = bs[b] || { shown: 0, unknown: 0, firstOk: 0, firstN: 0 };
  }
  function resetBandStat(b) { S.stats.bands[b] = { shown: 0, unknown: 0, firstOk: 0, firstN: 0 }; }
  function daysBetween(a, b) {
    var pa = a.split("-").map(Number), pb = b.split("-").map(Number);
    return Math.round((new Date(pb[0], pb[1] - 1, pb[2]) - new Date(pa[0], pa[1] - 1, pa[2])) / 86400000);
  }
  // 구간 안에서 고르게 n개 (미학습 우선, 위치는 조금씩 흔들어 같은 문제가 반복되지 않게)
  function sampleBand(band, n) {
    var pool = byModule.word.filter(function (it) { return it.band === band && !isSeen(it.id) && !S.skipped[it.id]; });
    if (pool.length < n) pool = byModule.word.filter(function (it) { return it.band === band; });
    var m = Math.min(n, pool.length), ids = [], used = {};
    for (var i = 0; i < m; i++) {
      var at = Math.min(pool.length - 1, Math.floor((i + Math.random()) * pool.length / m));
      while (used[at] && at > 0) at--;
      used[at] = true; ids.push(pool[at].id);
    }
    return ids;
  }
  function markSkipped(band, t) {
    var n = 0;
    MODULES.forEach(function (m) {
      if (!courseOn(m)) return;
      byModule[m].forEach(function (it) { if (it.band === band && !isSeen(it.id) && !S.skipped[it.id]) { S.skipped[it.id] = t; n++; } });
    });
    return n;
  }
  ...
  function placeRound() {
    var ids = sampleBand(sess.place.band, 5);
    if (!ids.length) return false;
    ids.forEach(function (id) { sess.queue.push(buildQuestion(id, "place")); });
    sess.place.rounds++;
    return true;
  }
  function placeStep() {
    var p = sess.place, ps = sess.placeStats[p.band] || { ok: 0, n: 0 };
  ...
  function finishPlacement(start) {
    var t = todayStr();
    setFocus(start); S.placed = true;
    S.level.lastCheck = t;
    S.level.history.push({ date: t, band: S.level.focus, kind: "init", ok: sess.ok, n: sess.queue.length, path: sess.place.path.slice() });
    Object.keys(sess.placeStats).forEach(function (b) { resetBandStat(Number(b)); });
    save();
    ...
    var lines = bands.map(function (b) { var ps = sess.placeStats[b]; return bandLabel(b) + ": " + ps.ok + "/" + ps.n; });
    rp.innerHTML = "<h3>구간별</h3><p class=\"tip\">" + lines.join(" · ") + "</p><p class=\"tip\"><b>" + esc(bandLabel(S.level.focus)) + "</b> 구간부터 시작해요. 레벨은 배우면서 계속 다시 재요 — 코스 지도에서 언제든 점검할 수 있어요.</p>";
  ...
  function startLevelCheck(band, kind) {
    ...
    if (!ids.length) return false;
    unlockAudio();
    sess = newSession("level", todayStr());
    sess.level = { band: band, kind: kind || "manual", ok: 0, n: 0 };
    sess.queue = ids.map(function (id) { return buildQuestion(id, "level"); });
    pushView("session");
    renderQuestion();
    return true;
  }
  function finishLevelCheck() {
    var L = sess.level, t = sess.t, lv = S.level, oldFocus = lv.focus;
    var passLine = A.levelPass || 8, failLine = A.levelFail || 6, msg, moved = false;
    if (L.band > lv.focus) {
      if (L.ok >= passLine) {
        var skipped = 0;
        for (var b = lv.focus; b < L.band; b++) skipped += markSkipped(b, t);
        setFocus(L.band); moved = true;
        msg = "통과! 이제 <b>" + esc(bandLabel(lv.focus)) + "</b> 구간부터 배워요." + (skipped ? " 건너뛴 " + skipped + "개는 하루 하나씩 확인해요 — 틀리면 보관함으로 와요." : "");
      } else {
        msg = esc(bandLabel(lv.focus)) + " 구간을 조금 더 다지면 좋겠어요 (" + L.ok + "/" + L.n + "). " + (A.levelDays || 14) + "일 뒤에 다시 제안할게요.";
      }
    } else {
      if (L.ok < failLine) {
        lv.boost = L.band; moved = true;
        msg = "<b>" + esc(bandLabel(L.band)) + "</b> 구간이 흔들려요 (" + L.ok + "/" + L.n + "). 당분간 새 단어의 일부를 이 구간에서 뽑아요.";
      } else {
        msg = "튼튼해요 ✓ " + esc(bandLabel(L.band)) + " 구간은 잘 알고 있어요 (" + L.ok + "/" + L.n + ").";
      }
    }
    lv.lastCheck = t;
    lv.history.push({ date: t, band: L.band, kind: L.kind, ok: L.ok, n: L.n, moved: moved });
    resetBandStat(oldFocus); resetBandStat(L.band);
    var d = dayRec(t); if (moved && d.set && !d.done && d.newStats.shown === 0) delete d.set; // 아직 안 시작한 오늘 세트는 새 구간으로 다시 뽑는다
    save();
    $("result-title").textContent = "레벨 점검 결과";
    animateScore($("score"), L.ok, L.n);
    var rp = $("result-place"); rp.classList.remove("hidden");
    rp.innerHTML = "<h3>" + esc(bandLabel(L.band)) + " 구간 · " + (L.band > oldFocus ? "승급 판정" : "보강 판정") + "</h3><p class=\"tip\">" + msg + "</p>"
      + "<p class=\"tip\">지금 레벨: <b>Lv " + lv.focus + " · " + esc(bandLabel(lv.focus)) + "</b></p>";
    $("result-grad").classList.add("hidden"); $("result-vault").classList.add("hidden");
    if (moved && L.band > oldFocus) setTimeout(function () { fxAt($("score"), fxColors(), 40, 7); sfxFanfare(); }, 300);
    show("view-result");
  }
  ...
  function bandCounts(b, modules) {
    ...
    ITEMS.forEach(function (it) {
      if (it.band !== b || modules.indexOf(it.module) < 0) return;
      c.total++;
      var r = rec(it.id);
      if (r && r.g) { if (r.due) c.grad++; else c.done++; }
      else if (r) c.vault++;
      else if (S.skipped[it.id]) c.skipped++;
      else if (isSeen(it.id)) c.learning++;
      else c.unseen++;
    });
    return c;
  }
  function renderMap() {
    ...
    Object.keys(CFG.bands || {}).map(Number).sort(function (a, b) { return a - b; }).forEach(function (b) {
      ...
      if (b < minBand()) { // 검색 전용 구간은 접어 둔다
        var c0 = bandCounts(b, MODULES);
        bn.innerHTML = "<span>L" + b + " · " + esc(bandLabel(b)) + "</span><span>" + c0.total + "개 · 검색 전용</span>";
        row.appendChild(bn); bar.appendChild(row); return;
      }
      var c = bandCounts(b, ["word"]);
      var tag = b === f ? " · 지금 여기" : b < f ? " · 통과 ✓" : "";
      if (lv.boost === b) tag = " · 보강 중";
      bn.innerHTML = "<span>L" + b + " · " + esc(bandLabel(b)) + tag + "</span><span>졸업 " + (c.grad + c.done) + "/" + c.total + "</span>";
      bn.addEventListener("click", function () { renderBandList(b); });
      var bp = el("div", "bp");
      [["g", c.grad + c.done], ["v", c.vault], ["s", c.learning + c.skipped]].forEach(function (seg) {
        var i = el("i", seg[0]); i.style.width = (c.total ? seg[1] / c.total * 100 : 0) + "%"; bp.appendChild(i);
      });
      var parts = [];
      if (c.vault) parts.push("보관함 " + c.vault);
      if (c.learning) parts.push("학습 중 " + c.learning);
      if (c.skipped) parts.push("건너뜀 " + c.skipped);
      parts.push("미학습 " + c.unseen);
    ...
    [["idiom", "숙어"], ["expr", "관용표현"]].forEach(function (pair) {
      ...
      tg.addEventListener("click", function () {
        S.courses[m] = { on: !on };
        var d = dayRec(todayStr()); if (d.set && !d.done && d.newStats.shown === 0) delete d.set; // 안 시작한 오늘 세트는 다시 뽑는다
        save(); renderMap();
      });
      bm.appendChild(tg); row.appendChild(bn); row.appendChild(bm); cb.appendChild(row);
    });
    if (cb.children.length) { rows.appendChild(el("h2", "maph", "코스")); rows.appendChild(cb); }
    show("view-list");
  }
  var lastBand = null;
  function renderBandList(b) {
    lastBand = b;
    var t = todayStr(), rows = $("list-rows"); rows.innerHTML = "";
    var act = $("btn-list-action"); act.classList.remove("hidden"); act.textContent = "← 코스 지도"; act.onclick = renderMap;
    var list = ITEMS.filter(function (it) { return it.band === b && courseOn(it.module); });
    $("list-title").textContent = "L" + b + " · " + bandLabel(b);
    $("list-sub").textContent = list.length + "개 — 누르면 상세, 보관함에 담을 수 있어요";
    list.forEach(function (it) { rows.appendChild(rowFor(it, t, function (x) { renderDetail(x, "band"); })); });
    show("view-list");
  }
  ...
  function importText(text) {
    try {
      ...
      if (!parsed || (parsed.v !== 1 && parsed.v !== 2) || !parsed.items) throw new Error("bad");
      S = normalize(migrateV1(parsed)); save();
  ...
  // ---------- 초기화 ----------
  function abortToHome() { sess = null; $("combo-badge").classList.add("hidden"); if (!S.placed && Object.keys(S.items).length === 0) show("view-place"); else renderHome(); }
  function renderPresetButtons() {
    var box = $("preset-btns"), seg = $("seg-profile"), row = $("row-profile");
    box.innerHTML = ""; seg.innerHTML = "";
    var ps = CFG.presets;
    if (!ps) { $("btn-place-start").hidden = false; row.hidden = true; return; }
    $("btn-place-start").hidden = true; row.hidden = false;
    Object.keys(ps).forEach(function (k) {
      var b = el("button", "btn primary big", ps[k].label + " — " + bandLabel(ps[k].start) + "부터 체크"); b.type = "button"; b.setAttribute("data-preset", k); box.appendChild(b);
      var s = el("button", "", ps[k].label); s.type = "button"; s.setAttribute("data-v", k); seg.appendChild(s);
    });
  }
  function init() {
    // S는 콘텐츠(legacy-ids.json 포함)를 읽은 뒤에 만든다 — v1 id 마이그레이션이 대응표를 필요로 한다.
    ...
    $("list-rows").addEventListener("click", function (e) {
      ...
      pushView("session"); startLevelCheck(b.getAttribute("data-band"), b.getAttribute("data-kind"));
    });
    // 온보딩 프리셋 (중학생/고등학생/성인) — 출발 구간과 기본 코스만 다르다 (v2 §6)
    renderPresetButtons();
    $("preset-btns").addEventListener("click", function (e) {
      var b = e.target.closest("button[data-preset]"); if (!b) return;
      var k = b.getAttribute("data-preset"); S.profile = k; save(); unlockAudio();
      pushView("session"); startPlacement(CFG.presets[k].start);
    });
    if (CFG.presets) bindSeg("seg-profile", function (v) { S.profile = v; pushView("session"); startPlacement(CFG.presets[v].start); });
    ...
    $("btn-place-start").addEventListener("click", function () { unlockAudio(); pushView("session"); startPlacement(CFG.defaultBand || 1); });
    $("btn-place-skip").addEventListener("click", function () { S.placed = true; setFocus(CFG.defaultBand || 1); save(); renderHome(); });
